...
const bannerSize = { width:"160" , height: "600"};
let pixelate = false;
...
(function () {
  ...
  function getSwipeDirection() {
      ...
      if (movementY <= -20 && Math.abs(movementX) < 160) {
          return 'RIGHT'
      }
      if (movementY >= 20 && Math.abs(movementX) < 160) {
          return 'LEFT'
  ...
  const farLeftPosition = -273
  const leftPosition = -133
  const centerPosition = 35
  const rightPosition = 201
  ...
  // // Video Setup HORIZONTALLY
  // function videoSetup() {
  //   let yPosition = 115    // Adjustment for top position of full carousel
  //   let xPosition = -230 * startScale // Spaciing - Width of 1 video thumbnail and then some
  //   let offsetX = 183     // Adjustment to make full carousel centered
  //   for (let i = 5 ; i >= 0 ; i--) {
  ...
  // }
    ...
    // Video Setup VERTICALLY
    function videoSetup() {
      let xPosition = 16    // Adjustment for RIGHT position of full carousel
      let yPosition = 20 * startScale // Spaciing - HEIGHT of 1 video thumbnail and then some
      let offsetY = 237     // Adjustment to make full carousel centered
      for (let i = 5 ; i >= 0 ; i--) {
        const iVid = document.querySelector(".vid"+(i+1))
        iVid.play()
        iVid.pause()
        gsap.set(iVid, {y: positions.y3 , x: positions.x3 , scale: startScale })
        // console.log(i, "x:", (i*xPosition)+offsetX, "y:", yPosition)
      }
    }
  ...
  // currentCount determines which video is in the center
    ...
    if (direction == "LEFT") {
      // gsap.to (leftVid , .25, { y: farRightPosition , zIndex: 20 , scale: startScale , ease: Power1.easeIn, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      // gsap.set(leftVid , { y: farLeftPosition , delay: .25})
      // gsap.to(leftVid , .25, { y: leftPosition , ease: Power1.easeOut , delay: .25})
      // gsap.to (rightVid , .5, { y: rightPosition , zIndex: 20 , ease: Power1.easeInOut})
      // gsap.to (rightVid , .35, { scale: startScale , ease: Power1.easeInOut, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})

      gsap.to (leftVid , .25, { x: positions.x5 , y: positions.y5 , zIndex: 20 , scale: startScale , ease: Power1.easeIn, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      gsap.set(leftVid , { x: positions.x1 , y: positions.y1 , delay: .25})
      gsap.to(leftVid , .25, { x: positions.x2 , y: positions.y2 , ease: Power1.easeOut , delay: .25})
      gsap.to (rightVid , .5, { x: positions.x4 , y: positions.y4 , zIndex: 20 , ease: Power1.easeInOut})
      gsap.to (rightVid , .35, { scale: startScale , ease: Power1.easeInOut, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})


    // Actions for side videos when MOVING RIGHT
    } else if (direction == "RIGHT") {
      gsap.to (leftVid , .5, {  x: positions.x2 , y: positions.y2 , zIndex: 20 , ease: Power1.easeInOut})
      gsap.to (leftVid , .35, { scale: startScale , ease: Power1.easeInOut, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      gsap.to (rightVid , .25, { x: positions.x1 , y: positions.y1 , zIndex: 20 , scale: startScale , ease: Power1.easeIn, boxShadow:  "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      gsap.set(rightVid , { x: positions.x5 , y: positions.y5 , delay: .25})
      gsap.to(rightVid , .25, { x: positions.x4 , y: positions.y4 , ease: Power1.easeOut , delay: .25})
    }

    // Actions for center video
    gsap.to (centerVid , .5, {  x: positions.x3 , y: positions.y3 , zIndex: 21 , ease: Power1.easeInOut,      
      onStart: function() {
        isAnimating = true
        leftVid.pause()
        rightVid.pause()
    ...
    gsap.to (centerVid , .5, { scale: .9 , ease: Back.easeOut , delay: .45 , boxShadow: "0px 6px 6px 6px rgba(0, 0, 0, 0.35)",
      onComplete: function() {
        isAnimating = false
        centerVid.play()
      }
  ...
  let split3 = new SplitText(".text6", { type: "chars", position: "relative" });
  ...
  maintl
    ...
    .set(".green_shape", { autoAlpha: 0 , scaleX: 2.2 }, "frame0")
    .set([split1.chars, split2.chars, split3.chars], { autoAlpha: 0 , scale: 0 }, "frame0")
    .set(centerVid , { zIndex: 21 } , "frame0")
    .to(".stage", 0.05, { autoAlpha: 1 , onStart: function(){ isAnimating = true }}, "frame0")
    .fromTo(".doomLogo" , 2 , {scale: .5  },{ scale: .65 , ease: Power0.easeNone}, "frame0")

    // Frame 1
    .add("frame1", "+=0")
    .to(".doomLogo, .text1" , .25 ,{ autoAlpha: 0 }, "frame1")
    .from(".background", 1 , { autoAlpha: 0 , scale: 1.2 }, "frame1")
    .from(".cta-button", .8 , { autoAlpha: 0 , scale:1.25 , ease: Power1.easeOut }, "frame1+=.1")
    .to([split3.chars], .5, { autoAlpha: 1, stagger: { each: .1, axis: "x", from: "start" } , scale: 1 , ease: Back.easeOut }, "frame1+=1")
    .to(split2.chars, .45, { autoAlpha: 1, stagger: { each: .08, axis: "x", from: "end" } , scale: 1 , ease: Back.easeOut }, "frame1+=1")
    .to([split1.chars], .55, { autoAlpha: 1, stagger: { each: .1, axis: "x", from: "start" } , scale: 1 , ease: Back.easeOut }, "frame1+=1")
    .to(".green_shape", .8 , { autoAlpha: 1 , scaleX :1 , ease: Power1.easeOut }, "frame1+=1.4")

    // Frame 2
    .add("frame2", 2.2)
    .from(".carouselVid1", .5, { x: positions.x5 , scale: startScale , stagger: .25 , ease: Power1.easeInOut } , "frame2")
    .from(".carouselVid1", .5, { y: positions.y5 , stagger: .25 , ease: Power0.easeNone } , "frame2")
    .to(".carouselVid1", .5, { x: positions.x1 , scale: startScale , stagger: .25 , ease: Power1.easeInOut } , "frame2+=0.5")
    .to(".carouselVid1", .5, { y: positions.y1  , stagger: .25 , ease: Power0.easeNone } , "frame2+=0.5")
    .from(vid3, .5, { x: positions.x5 , scale: startScale , stagger: .25 , ease: Power1.easeInOut } , "frame2+=.75")
    .from(vid3, .5, { y: positions.y5 , stagger: .25 , ease: Power0.easeNone } , "frame2+=.75")
    .to(vid3, .25, { x: positions.x2 , scale: startScale , ease: Power1.easeInOut } , "frame2+=1.25")
    .to(vid3, .25, { y: positions.y2 , ease: Power1.easeOut } , "frame2+=1.25")
    .fromTo(vid2, .25, { x: positions.x5 , scale: startScale } , { x: positions.x4 , scale: startScale , ease: Power0.easeNone } , "frame2+=1")
    .fromTo(vid2, .25, { y: positions.y5 } , { y: positions.y4 , ease: Power0.easeNone } , "frame2+=1")
    .to(vid2, .25, { x: positions.x3 , scale: startScale , ease: Power1.easeOut } , "frame2+=1.25")
    .to(vid2, .25, { y: positions.y3 , ease: Power1.easeOut } , "frame2+=1.25")
    .fromTo(vid1, .5, { x:positions.x5 , scale: startScale } , {x: positions.x4 , scale: startScale , ease: Power1.easeInOut } , "frame2+=1")
    .fromTo(vid1, .5, { y: positions.y5 } , { y: positions.y4 , ease: Power1.easeOut } , "frame2+=1")

    .set (centerVid , { zIndex: 21 } , "frame3")
    .to (centerVid , .5, { scale: .9 , ease: Back.easeOut , boxShadow: "0px 6px 6px 6px rgba(0, 0, 0, 0.35)",
      onComplete: function() {
        isAnimating = false;
        toggleAutoplay(autoPlay)
        centerVid.play()
