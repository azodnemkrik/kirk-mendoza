...
const bannerSize = { width:"728" , height: "90"};
let pixelate = false;
...
(function () {
  ...
  const farLeftPosition = 265
  const leftPosition = 275
  const centerPosition = 354
  const rightPosition = 433
  const farRightPosition = 443
  const startScale = .70
  ...
  // Video Setup
  function videoSetup() {
    let yPosition = 11    // Adjustment for top position of full carousel
    let xPosition = -110 * startScale // Spaciing - Width of 1 video thumbnail and then some
    let offsetX = 430     // Adjustment to make full carousel centered
    for (let i = 5 ; i >= 0 ; i--) {
  ...
  // currentCount determines which video is in the center
    ...
    if (direction == "LEFT") {
      gsap.to (leftVid , .25, { x: farRightPosition , zIndex: 20 , scale: startScale , autoAlpha: 0 ,  ease: Power1.easeIn, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      gsap.set(leftVid , { x: farLeftPosition  , autoAlpha: 0 , delay: .25})
      gsap.to(leftVid , .25, { x: leftPosition , autoAlpha: 1 , ease: Power1.easeOut , delay: .25})
      gsap.to (rightVid , .5, { x: rightPosition , autoAlpha: 1 , zIndex: 20 , ease: Power1.easeInOut})
    ...
    // Actions for side videos when MOVING RIGHT
      ...
      gsap.to (leftVid , .5, { x: leftPosition , zIndex: 20 , autoAlpha: 1 , ease: Power1.easeInOut})
      gsap.to (leftVid , .35, { scale: startScale , ease: Power1.easeInOut, boxShadow: "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      gsap.to (rightVid , .25, { x: farLeftPosition , zIndex: 20 , scale: startScale , autoAlpha: 0 , ease: Power1.easeIn, boxShadow:  "0px 0px 0px 0px rgba(0, 0, 0, 0)"})
      gsap.set(rightVid , { x: farRightPosition , autoAlpha: 0 , delay: .25})
      gsap.to(rightVid , .25, { x: rightPosition , autoAlpha: 1 , ease: Power1.easeOut , delay: .25})
    }
  ...
  maintl
    ...
    .fromTo(".doomLogo" , 2 ,{ scale: .4 },{ scale: .5 , ease: Power0.easeNone}, "frame0")

    // Frame 1
    .add("frame1", "+=0")
    .to(".doomLogo, .text1" , .25 ,{ autoAlpha: 0 }, "frame1")
    .set("#ember_canvas", { autoAlpha: 0 }, "frame1")
    ...
    .from(".cta-button", .8 , { autoAlpha: 0 , x :25 , ease: Power4.easeOut }, "frame1+=1")

    // Frame 2
    .add("frame2", 1.5)
    .from(".carouselVid", 2, { x: "+=1500", ease: Power4.easeInOut } , "frame2")
    .to(".vid6, .vid5, .vid4", .45, { autoAlpha: 0, stagger: .1 , ease: Power4.easeInOut } , "frame2+=1")
