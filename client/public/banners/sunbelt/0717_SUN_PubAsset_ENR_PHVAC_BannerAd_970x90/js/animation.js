...
Enabler.setExpandingPixelOffsets("0", "0", "728", "415")
...
function enablerInitHandler() {
	...
	maintl
		.set('.expanded', { y: -325 }) // Changed to 0 for testing
		.set('.btn-close-over, .close-button', { autoAlpha: 0 })
	...
	function setupMouseEvents() {
		...
		stage.addEventListener('mousemove', (e) => {
			...
			// Left half
			if (mouseY > 100 && mouseY < 300) { // Only scroll if mouse is in the top 200px
				if (mouseX < halfWidth) {
					scrollDirection = -1
	...
	closeButton.addEventListener('mouseover', function () {
		gsap.to('.btn-close', 0.3, { autoAlpha: 0, ease: Power2.easeInOut });
		gsap.to('.btn-close-over', 0.2, { autoAlpha: 1, ease: Power2.easeInOut });
	});
	closeButton.addEventListener('mouseout', function () {
		gsap.to('.btn-close', 0.2, { autoAlpha: 1, ease: Power2.easeInOut });
		gsap.to('.btn-close-over', 0.3, { autoAlpha: 0, ease: Power2.easeInOut });
	});
	...
	document.querySelector(".close-button").addEventListener(myEvent, actionClickHandler);
	...
	function expandStartHandler() {
		...
		gsap.to('.banner', .65, { height: 415, ease: Power2.easeInOut })
		...
		gsap.to('.close-button', .25, { autoAlpha: 1, ease: Power2.easeInOut, delay: .25 })
	...
	function collapseStartHandler() {
		...
		// When animation finished must call
		gsap.to(".expanded", .65, { y: -325, ease: Power2.easeInOut, onComplete: Enabler.finishCollapse() })
		gsap.to('.expand-button', .25, { autoAlpha: 1, ease: Power2.easeInOut, delay: .25 })
		gsap.to('.close-button', .15, { autoAlpha: 0, ease: Power2.easeInOut })
